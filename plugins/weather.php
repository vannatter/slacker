<?php
	...
	class weather extends Slacker {
		...
		function __construct() {
			...
			$data = $this->run_curl("https://api.wunderground.com/api/" . $this->config['wunderground_api_key']  . "/geolookup/conditions/q/IA/" . $this->command_text . ".json", "GET");
			$data_decoded = json_decode($data);
			
			if (!isset($data_decoded->response->error)) {
				$this->webhook_setting("icon_url", $data_decoded->{'current_observation'}->{'icon_url'});
